...
const STORAGE_KEY = 'newsletter_unipop_v4';
...
function normalizeCourse(raw, index) {
  ...
  const description = normalizeSpaces(first(raw, ['description', 'resume', 'résumé', 'details', 'objectif', 'objectifs'])) || 'Découvrez ce nouveau cours proposé par UniPop.';
...
async function loadCourses() {
  ...
  try {
    ...
    $('jsonStatus').textContent = 'Données filtrées avec code UNIPOP / mail / nom UniPop';
    fillCategories();
    renderCourses();
    renderPreview();
  } catch (error) {
...
function filteredCourses() {
  ...
  const rows = allCourses.filter(course => {
    const haystack = [course.code, course.title, course.category, course.date, course.place, course.trainer].join(' ').toLowerCase();
    return haystack.includes(query) && (!category || course.category === category);
  });
  rows.sort((a, b) => {
...
function renderCourses() {
  ...
  $('courseCount').textContent = `${rows.length} cours disponibles`;
...
function addBlock(type) {
  const block = { id: Date.now().toString(), type, title: '', text: '', button: 'Découvrir', url: 'https://www.unipop.lu' };
...
<article class="featured-course">
  <div class="featured-img">${previewImage(course)}</div>
  <div class="featured-content">
    <h3>${esc(course.title)}</h3>
    <div class="mail-badges"><span class="mail-category">${esc(course.category || 'UniPop')}</span><span class="mail-new">Nouveau</span></div>
...
function mailCourse(course) {
  const image = courseImages[course.id]
    ? `<img src="${courseImages[course.id]}" alt="${esc(course.title)}" style="width:100%;height:198px;object-fit:cover;display:block;border-radius:9px;">`
    : `<div style="height:198px;border-radius:9px;background:#0d1f59;color:#ffffff;font-size:56px;font-weight:900;text-align:center;line-height:198px;">A</div>`;

  return `
<div style="border:1px solid #e3e8f1;border-radius:12px;padding:14px;margin:0 28px 18px;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td width="270" style="vertical-align:middle;padding-right:28px;">${image}</td>
      <td style="vertical-align:middle;">
        <h3 style="margin:0 0 10px;color:#0d1f59;font-size:23px;line-height:1.25;">${esc(course.title)}</h3>
        <div style="margin:10px 0 18px;"><span style="background:#edf4ff;color:#073684;border-radius:999px;padding:7px 11px;font-size:13px;font-weight:900;text-transform:uppercase;">${esc(course.category || 'UniPop')}</span> <span style="background:#fff5d7;color:#9b6600;border-radius:999px;padding:7px 11px;font-size:13px;font-weight:900;">Nouveau</span></div>
        <p style="color:#42526d;margin:8px 0;">📅 ${esc(formatDate(course.date))}</p>
        <p style="color:#42526d;margin:8px 0;">📍 ${esc(course.place)}</p>
      </td>
    </tr>
  </table>
</div>`;
