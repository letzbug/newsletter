      ...
      ${isContentBlock ? `
        <div class="media-editor-fields">
          ...
          <label>Vidéo YouTube / Vimeo / UniPop
            <input class="media-video-input" type="url" placeholder="https://... (optionnel)" value="${esc(block.video || '')}">
          </label>
          ...
          <p class="media-help">Pour Mailjet, les images doivent être des liens publics HTTPS. Pour une vidéo, l’e-mail affiche une image cliquable ou un bouton Play.</p>
        </div>
      ` : ''}
...
function contentBlockMediaHtml(block, compact = false, mailjetExport = false) {
  ...
  const videoUrl = ensureExternalUrl(block.video || '');
  const uploadedImage = mailjetImageUrl(block.image || '');
  const youtubeId = youtubeVideoId(videoUrl);
  const youtubeImage = youtubeThumbnail(videoUrl);
  const thumbnail = uploadedImage || youtubeImage;

  // Builder / Aperçu: YouTube wird direkt im Newsletter-Fenster abgespielt.
  // Mailjet Export: kompatibel als Thumbnail + Play-Button.
  if (videoUrl && youtubeId && !mailjetExport) {
    return `
      <iframe
        ...
        height="360"
        frameborder="0"
        style="display:block;width:100%;max-width:620px;height:360px;border:0;border-radius:12px;margin:0 auto ${compact ? '0' : '16px'};"
          ...
          <tr>
            <td align="center" style="padding:0 0 ${compact ? '0' : '12px'};">
              <a href="${videoHref}" target="_blank" style="display:block;text-decoration:none;color:inherit;">
                <img src="${esc(thumbnail)}" alt="${title}" width="620" style="display:block;width:100%;max-width:620px;height:auto;border:0;outline:none;text-decoration:none;border-radius:12px;margin:0 auto;">
              </a>
            </td>
          </tr>
          <tr>
            <td align="center" style="padding:0 0 ${compact ? '0' : '14px'};">
              <a href="${videoHref}" target="_blank" style="display:inline-block;background:#0d1f59;color:#ffffff;text-decoration:none;border-radius:999px;padding:9px 16px;font-size:14px;font-weight:900;font-family:Arial,Helvetica,sans-serif;">▶ Voir la vidéo</a>
            </td>
          </tr>
        </table>`;
    }
  ...
  if (thumbnail) {
    return `<img src="${esc(thumbnail)}" alt="${title}" width="620" style="display:block;width:100%;max-width:620px;height:auto;border:0;outline:none;text-decoration:none;border-radius:12px;margin:0 auto ${compact ? '0' : '16px'};">`;
  }
