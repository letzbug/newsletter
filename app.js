...
function buildNewsletterHtml(mailjet = false) {
  ...
  return `
<div class="mail-container" style="max-width:760px;margin:0 auto;background:#ffffff;font-family:Arial,Helvetica,sans-serif;color:#071746;">
...
$('previewBtn').addEventListener('click', () => {
  ...
  const previewElement = $('newsletterPreview');
  if (!previewElement) {
    alert('Aperçu introuvable.');
    return;
  }

  const previewWindow = window.open('', 'newsletterPreviewWindow', 'width=1050,height=900,scrollbars=yes,resizable=yes');
  if (!previewWindow) {
...
<head>
  ...
  <style>
    body{margin:0;padding:30px;background:#eef2f7;font-family:Arial,Helvetica,sans-serif;}
    .app-shell,.topbar,.sidebar,.workspace,.editor-column,.preview-heading{display:none!important;}
    .newsletter-preview{display:block!important;width:100%!important;max-width:760px!important;margin:0 auto!important;}
