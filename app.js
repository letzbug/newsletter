const DATA_URL = 'https://letzbug.github.io/franks_magic/data/trainings.json';
...
function strictUniPopMarker(c) {
  // Même logique que le signage UniPop : plus souple que l'ancien filtre.
  // 1) organisateur.code === UNIPOP
  const orgCode = (c?.organisateur?.code || c?.organizer?.code || c?.organizerCode || '')
    .toString()
    .trim()
    .toUpperCase();
  if (orgCode) return orgCode === 'UNIPOP';

  // 2) certains cours UniPop possèdent ce champ
  if (c?.categorieCodeUnipop) return true;

  // 3) adresseCours.nom contient Université Populaire
  const addrName = (c?.adresseCours?.nom || c?.adresseCours?.name || '')
    .toString()
    .toLowerCase();
  if (addrName.includes('universit') && addrName.includes('populaire')) return true;

  // 4) fallback ancien format : recherche récursive code/mail/nom UniPop
  let found = false;
  const walk = (node) => {
...
function nestedFirst(obj, paths) {
...
function shortLieuFromRaw(raw) {
  const direct = normalizeSpaces(nestedFirst(raw, [
    'site', 'lieu', 'location', 'SALLE', 'salle', 'adresse', 'localisation',
    'adresseCours.nom', 'adresseCours.localite', 'adresseCours.localité'
  ]));
  return direct || 'Université Populaire Belval';
}

function getCourseCode(raw, index = 0) {
  return first(raw, ['coursCode', 'coursId', 'codeCours', 'courseCode', 'idCours', 'id', 'ID']) || `course-${index}`;
}

function normalizeCourse(raw, index) {
  const code = getCourseCode(raw, index);
  const id = code || `course-${index}`;

  const title = normalizeSpaces(first(raw, [
    'intitule', 'INTITULE', 'titre', 'Titre', 'title', 'libelle', 'Libelle', 'name'
  ])) || code;

  const category = normalizeSpaces(first(raw, [
    'categorieNom', 'categorie', 'catégorie', 'category', 'theme', 'discipline', 'domaine', 'categorieCodeUnipop'
  ]));

  const dateDebut = first(raw, ['dateDebut', 'DEBUT', 'debut', 'date', 'start', 'date_debut', 'begin']);
  const dateFin = first(raw, ['dateFin', 'FIN', 'fin', 'end', 'date_fin']);
  const date = dateDebut || dateFin;
  const dateLabel = formatDateRange(dateDebut, dateFin);

  const place = shortLieuFromRaw(raw);

  const trainer = normalizeSpaces(first(raw, [
    'formateur', 'Formateur', 'enseignant', 'trainer', 'intervenant', 'nomFormateur'
  ]));

  const description = normalizeSpaces(first(raw, [
    'description', 'resume', 'résumé', 'details', 'objectif', 'objectifs', 'contenu', 'presentation', 'publicCible'
  ])) || 'Découvrez ce nouveau cours proposé par UniPop.';

  const link = first(raw, ['url', 'lien', 'link', 'permalink']) || '';

  return { id, code, title, category, date, dateDebut, dateFin, dateLabel, place, trainer, description, link, raw };
}
...
async function loadCourses() {
  ...
  try {
    ...
    $('jsonStatus').textContent = `UniPop uniquement · ${allCourses.length} cours`;
    fillCategories();
    renderCourses();
    renderPreview();
  } catch (error) {
...
function renderCourses() {
  ...
  rows.forEach(course => {
    ...
    node.querySelector('.course-meta').textContent = [course.dateLabel || formatDate(course.date), course.place, course.trainer].filter(Boolean).join(' · ');
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:separate;background:#ffffff;border:1px solid #e3e8f1;border-radius:12px;overflow:hidden;box-shadow:0 14px 28px rgba(7,23,70,0.16);">
  ...
  <tr>
    <td style="padding:16px 14px 14px;font-family:Arial,Helvetica,sans-serif;">
      ...
      <p style="margin:4px 0;color:#42526d;font-size:13px;line-height:1.3;font-family:Arial,Helvetica,sans-serif;">🗓️ ${esc(course.dateLabel || formatDate(course.date))}</p>
...
function contentBlockMediaHtml(block, compact = false, mailjetExport = false, targetUrl = '') {
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border-collapse:collapse;">
          <tr>
            <td align="center" style="padding:0 0 ${compact ? '0' : '12px'};">
              <a href="${videoHref}" target="_blank" style="display:block;text-decoration:none;color:inherit;">
  ...
  if (thumbnail) {
    const imgHtml = `<img src="${esc(localAssetImageUrl(thumbnail))}" alt="${title}" width="620" style="display:block;width:100%;max-width:620px;height:auto;border:0;outline:none;text-decoration:none;border-radius:12px;margin:0 auto ${compact ? '0' : '16px'};">`;
    return targetUrl ? `<a href="${esc(targetUrl)}" target="_blank" style="display:block;text-decoration:none;color:inherit;">${imgHtml}</a>` : imgHtml;
  }
...
function mailjetContentBlock(block, mailjetExport = false) {
  ...
  const target = (block.clickable !== false) ? blockTargetUrl(block) : '';
  const media = contentBlockMediaHtml(block, position !== 'top', mailjetExport, target);

  const titleHtml = title
    ? `<h3 style="margin:0 0 8px;color:#0d1f59;font-size:${block.type === 'quote' ? '20px' : '18px'};line-height:1.2;font-weight:900;font-family:Arial,Helvetica,sans-serif;">
        ${target ? `<a href="${esc(target)}" target="_blank" style="color:#0d1f59;text-decoration:none;">${title}</a>` : title}
      </h3>`
    : '';

  const textHtml = text
    ? `<p style="margin:0;color:#344054;font-size:15px;line-height:1.5;font-family:Arial,Helvetica,sans-serif;">${text}</p>
       ${target ? `<p style="margin:14px 0 0;"><a href="${esc(target)}" target="_blank" style="color:#0d1f59;font-weight:900;text-decoration:none;font-family:Arial,Helvetica,sans-serif;">En savoir plus →</a></p>` : ''}`
    : '';
