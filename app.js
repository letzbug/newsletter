...
function strictUniPopMarker(c) {
  const orgCode = (c.organisateur && c.organisateur.code)
    ? String(c.organisateur.code).trim().toUpperCase()
    : '';
  ...
  const addrName = (c.adresseCours && (c.adresseCours.nom || ''))
    ? String(c.adresseCours.nom).toLowerCase()
    : '';
