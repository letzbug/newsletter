...
const FIXED_HERO_IMAGE = 'assets/Newsletter_Header.png';
...
function isUnipopCourse(item){
  const organizerCode = clean(item?.organisateur?.code).toUpperCase();
  const organizerName = clean(item?.organisateur?.nom).toLowerCase();
  const coursePlace = clean(item?.adresseCours?.nom).toLowerCase();
  const categoryUnipop = clean(item?.categorieCodeUnipop);
  return organizerCode === 'UNIPOP'
    || organizerName.includes('unipop')
    || coursePlace.includes('unipop')
    || coursePlace.includes('université populaire')
    || !!categoryUnipop;
}
...
function mapCourse(item, index){
  ...
  const trainer = first(item, ['formateur','formateurNom','trainer','enseignant','intervenant','nomFormateur']) || (Array.isArray(item?.enseignants) ? item.enseignants.map(e=>[e.prenom,e.nom].filter(Boolean).join(' ')).join(', ') : '');
  const location = first(item, ['site','lieu','location','SALLE','salle','commune','place']) || first(item?.adresseCours, ['nom','localite']);
  const url = first(item, ['url','link','permalink','detailsUrl','registrationUrl','onlineRegistrationUrl']) || $('catalogUrl')?.value || 'https://www.unipop.lu';
...
async function loadCourses(){
  ...
  try{
    ...
    allCourses = flattenData(data).filter(isUnipopCourse).map(mapCourse).filter(c => c.title);
    $('jsonStatus').textContent = `JSON chargé depuis GitHub · filtre UniPop actif.`;
...
function loadState(){
  ...
  $('newsletterTitle').value = s.title || $('newsletterTitle').value; $('newsletterSubtitle').value = s.subtitle || $('newsletterSubtitle').value; if($('heroImage')) $('heroImage').value = FIXED_HERO_IMAGE; $('catalogUrl').value = s.catalog || $('catalogUrl').value;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  ['newsletterTitle','newsletterSubtitle','catalogUrl'].forEach(id=>$(id).addEventListener('input',()=>{renderPreview(); saveState();}));
